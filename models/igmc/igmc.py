...
# Based on https://github.com/LspongebobJH/dgl/blob/pr1815_igmc/examples/pytorch/igmc/
...
class IGMC(nn.Module):
    ...
    def __init__(self, in_feats, gconv=RelGraphConv, latent_dim=(32, 32, 32, 32),
                 num_relations=1, num_bases=2, regression=False, edge_dropout=0.2,
    ...
    def forward(self, block):
        ...
        for conv in self.convs:
            ...
            x = torch.tanh(conv(block, x, block.edata['etypes'],
    ...
    def loss(self, pos, neg):
        pos_score = self.forward(pos)
        neg_score = self.forward(neg)

        return -self.loss_fn(pos_score - neg_score).sum(), pos_score > neg_score
...
def edge_drop(graph, edge_dropout=0.2, training=True):
    ...
    src, _ = graph.edges()
    to_drop = src.new_full((graph.number_of_edges(),), edge_dropout, dtype=torch.float)
    to_drop = torch.bernoulli(to_drop).to(torch.bool)
